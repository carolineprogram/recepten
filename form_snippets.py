...
import math
from utils import get_all_recipe_names, get_all_types, get_types, get_all_ingredients, get_ingredients, insert_recipe, update_recipe, update_ingredients, update_types
...
def select_recipe(form_key="Select Recipe", button_label="Info"):
    ...
    with st.form(key=form_key, clear_on_submit=True):
        selected_recipe = st.selectbox("Kies recept?", recipe_names)
...
def fill_in_recipe(form_key, button_label, recipe_id, recipe_name, beschrijving, bron, locatie, gemaakt):
    with st.form(key=form_key, clear_on_submit=True):
        ...
        types = get_all_types()
        if recipe_id:
            types_in_db = get_types(recipe_id) #resultaat is list met lists[type_id, type_naam] voor bepaald recept
